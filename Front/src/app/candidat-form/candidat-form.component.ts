...
export class CandidatFormComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    private candidatService: ProfileService,
  ...
  onSubmit(): void {
    ...
    if (this.currentFile) {
      ...
      this.http.post<any>('http://localhost:8080/api/postulation?nom=' + this.form.value.nom + '&prenom=' + this.form.value.prenom + '&region=' + this.form.value.region + '&idj=' + this.offreId + '&idc=' + this.tokenStorage.getUser().id + '&tel=' + this.form.value.tel + '&civilite=' + this.form.value.civilite + "&diplomes=" + diplomasString + '&domaine=' + this.form.value.domaine + '&experienceProf=' + this.form.value.expProf + '&competences=' + this.convertListToString(this.selectedCompetences), cv).subscribe({
        next: data => {
          console.log(data)
